...
# 6) Plot the residuals after Box-Cox transformation
```{r}
residuals_data <- residuals(lm_model1)
...
```{r}
lm_model2 <- lm(price ~ . - diamond_classification, data = data_with_boxcox)
```

# 13) Plot the residuals after Box-Cox transformation
```{r}
residuals_data <- residuals(lm_model2)
...
# 14) Breusch-Pagan test for heteroscedasticity
```{r}
bptest_result <- bptest(lm_model2)
print(bptest_result)
```

# 15) Calculate Cook's distance
```{r}
